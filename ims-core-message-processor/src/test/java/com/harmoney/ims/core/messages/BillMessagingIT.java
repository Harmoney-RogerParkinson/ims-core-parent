 ...
 * 
 * It uses the loan__Loan_account_Due_Details__c table and updates loan__Remarks__c to trigger the pushTopic.
 * 
...
public class BillMessagingIT {
	
    private static final Logger log = LoggerFactory.getLogger(BillMessagingIT.class);
	...
	public void testSubscription() throws ConnectionException, InterruptedException {
		...
		MessageHandler messageHandler = messageHandlerMap.getMessageHandler("/topic/BILL");
	...
	private int updateInvestorLoanTransaction() throws ConnectionException {
		...
		QueryResult qr = partnerConnection.query("SELECT Id,loan__Remarks__c FROM loan__Loan_account_Due_Details__c");
		qr.getSize();
